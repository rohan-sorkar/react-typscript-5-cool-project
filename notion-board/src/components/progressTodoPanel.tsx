...
interface PropTypes {
  ...
  deleteTask: (id: string, key: "progress") => void;
}

const ProgressTodoPanel = ({ tasks, updateTask, deleteTask }: PropTypes) => {
  ...
  const handleUpdateTask = ({
    ...
    if (event.key === "Enter") {
      ...
      updateTask({ id, key: "progress", data: updateState });
      setIsUpdate(false);
    }
  };
    ...
    <div className="w-[26rem]">
      <div>
        <div className="flex items-center gap-2">
          <img className="w-10" src={inProgressImg} />
          <h1 className="text-xl font-bold text-slate-700">In Progress</h1>
          <h4 className="text-xl font-bold bg-slate-200 px-2 rounded-md text-slate-600">
            {tasks.length}
          </h4>
        </div>
        <p className="text-lg mt-3 ml-2">This is actively being worked on</p>
      </div>
                ...
                {(provided, snapshot) => (
                  <div ref={provided.innerRef} {...provided.dragHandleProps} {...provided.draggableProps} className="my-3">
                    {isUpdate && updateState?.id === task.id ? (
                      ...
                      <div className="flex justify-between px-4 bg-slate-50 py-3 capitalize rounded-lg">
...
export default ProgressTodoPanel;
